...
1. **OddsBandFilter Calibration Data Source**
   - What we know: D-05 says "training period bet data". `BacktestEngine.run()` only receives test dates.
   - What's unclear: Whether training-period bet history should be generated by a separate mini-backtest on training data, or whether the filter should be calibrated from OOF prediction data that already exists from model training.
   - Recommendation: The simplest approach is to have `run_backtest.py` pass training-period bet history (generated by running `get_win_candidates()` on training data with known outcomes) to the `OddsBandFilter.calibrate()` method before the test backtest starts. Alternatively, the `OddsBandFilter` could be calibrated inside `engine.run()` using a warm-up window of the first N races.

2. **Statistics Accumulation vs COLLAPSED Skip Order**
   - What we know: `recent_stats_list` is appended at engine.py:966-991, which is inside the race loop but after bet settlement. If COLLAPSED skip fires at line ~688, stats for that race are never accumulated.
   - What's unclear: Whether the regime detector needs market stats from COLLAPSED races to transition back to CONSERVATIVE/AGGRESSIVE.
   - Recommendation: Move `recent_stats_list.append()` to BEFORE the COLLAPSED check, or accumulate stats regardless of COLLAPSED state. This ensures the regime detector has full visibility.
