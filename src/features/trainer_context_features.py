...
class TrainerContextFeatures:
    ...
    def _load_stats(self) -> pd.DataFrame:
        if self._stats_cache is None:
            from db.readers import load_trainer_stats

            self._stats_cache = load_trainer_stats(self.store)
    ...
    def compute(self, entry_df: pd.DataFrame) -> pd.DataFrame:
        """entry_df (race_id, umaban, chokyosicode, race_date) -> 調教師特徴量DataFrame
        ...
        # chokyosicode の型を統一 (entry: float64, stats: object の不一致を回避)
        entry_df["chokyosicode"] = entry_df["chokyosicode"].astype(str)
        ...
        result["trainer_wr_overall"] = np.where(total == 0, np.nan, (wins + 1) / (total + 11))
        ...
        result["trainer_wr_distance"] = np.where(ky1_t == 0, np.nan, (ky1_w + 1) / (ky1_t + 11))
        ...
        result["trainer_wr_venue"] = np.where(j5_t == 0, np.nan, (j5_w + 1) / (j5_t + 11))
