...
class PlaceSelectionGateModel:
    ...
    RANK_COL = "place_gate_rank"
    GAP_COL = "place_gate_score_gap"
    ...
    RUNNER_UP_PROB_COL = "runner_up_place_selection_prob"
    RUNNER_UP_EDGE_COL = "runner_up_place_selection_edge"
    RUNNER_UP_ODDS_COL = "runner_up_fukuoddslow"
    ...
    def _score_frame_from_tables(
        ...
        prepared = ensure_place_selection_columns(df)
        prob_bins = _bucketize(prepared["place_selection_prob"], list(tables["prob_edges"]))
        edge_bins = _bucketize(prepared["place_selection_edge"], list(tables["edge_edges"]))
    ...
    def _pass_mask(
        ...
        prob = _numeric_or_nan(df, "place_selection_prob")
        edge = _numeric_or_nan(df, "place_selection_edge")
        odds = _numeric_or_nan(df, "fukuoddslow")
        return (
            prob.ge(min_prob)
            & edge.ge(min_edge)
            & odds.gt(0.0)
                    ...
                    for edge_min in edge_values:
                        for odds_max in odds_values:
                            for market_min in market_values:
                                ...
                                profit = float((selected["realized_place_roi"] - 1.0).sum())
                                roi = float(selected["realized_place_roi"].mean())
    ...
    def soft_pass_mask(
        ...
        *,
        ...
        prob = pd.to_numeric(prepared["place_selection_prob"], errors="coerce")
        edge = pd.to_numeric(prepared["place_selection_edge"], errors="coerce")
        odds = _numeric_or_nan(prepared, "fukuoddslow")
        hard_mask = prepared[self.PASS_COL].fillna(False).astype(bool)
        outer_mask = (
            (edge >= edge_floor)
